...
} from './server/comment-repository';

type JsonObject = Record<string, unknown>;

interface RuntimePreviewSnapshot {
  endpoints: ReadonlyMap<string, Buffer>;
  generationId: string;
...
const runtimeDataDirectory = resolve(
  process.env.LOCAL_DATA_DIR || resolve(process.cwd(), 'private-runtime'),
);
...
function loadRuntimePreviewSnapshot(projectRoot: string): RuntimePreviewSnapshot {
  const paths = {
  ...
  // research.json is the builder's commit marker. The endpoint bytes are held
  // for the lifetime of this process, so a running server can never expose a
  // partially replaced on-disk endpoint set.
  ...
  const runtimeResearch = parseJsonObject('runtime research.json', researchAfter);
  const embeddedResearch = requireObject('compiled research.json', research as unknown);
  const runtimeMeta = requireObject('runtime research metadata', runtimeResearch._meta);
  ...
  const parsedEndpoints = {
    persons: parseJsonObject('runtime persons.json', endpointFiles['/data/persons.json']),
    events: parseJsonObject('runtime events.json', endpointFiles['/data/events.json']),
    timeline: parseJsonObject('runtime timeline.json', endpointFiles['/data/timeline.json']),
    sources: parseJsonObject('runtime sources.json', endpointFiles['/data/sources.json']),
  };
  ...
  const events = requireArray('runtime research events', runtimeResearch.events);
  const sources = requireArray('runtime research sources', runtimeResearch.sources);
  requireExact(
...
if (!loopbackHosts.has(hostname)) {
  throw new Error(`Local preview refuses non-loopback host: ${hostname}`);
}
if (process.env.COZE_PROJECT_ENV?.toUpperCase() === 'PROD') {
  throw new Error('Local preview refuses COZE_PROJECT_ENV=PROD');
}
const runtimeSnapshot = loadRuntimePreviewSnapshot(resolve(process.cwd()));
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      if (pathname === '/api/local/analytics') {
        await handleLocalAnalytics(req, res);
        return;
      }
      if (pathname === '/api/local/messages') {
        await handleLocalMessage(req, res);
        return;
      }
      if (pathname === '/api/local/insights') {
        handleLocalInsights(req, res);
        return;
      }
      if (pathname === '/api/local/inbox') {
        handleLocalInbox(req, res);
        return;
      }
      if (pathname === '/api/local/novel-comments') {
        await handleNovelComments(req, res, requestUrl);
        return;
      }
      if (pathname === '/api/local/novel-comments/inbox') {
        handleNovelCommentInbox(req, res, requestUrl);
        return;
      }
      if (pathname === '/api/local/novel-comments/moderate') {
        await handleNovelCommentModeration(req, res);
        return;
      }
      if (pathname === '/api/local/corpus-hits') {
        handleLocalCorpusHits(req, res, requestUrl);
        return;
      }
      const endpoint = runtimeSnapshot.endpoints.get(pathname);
      if (endpoint !== undefined && (req.method === 'GET' || req.method === 'HEAD')) {
        res.statusCode = 200;
        res.setHeader('Content-Type', 'application/json; charset=utf-8');
        res.setHeader('Content-Length', String(endpoint.length));
        res.setHeader('X-Su-Kaiyuan-Generation', runtimeSnapshot.generationId);
        res.setHeader('X-Su-Kaiyuan-Research-SHA256', runtimeSnapshot.researchSha256);
        res.end(req.method === 'HEAD' ? undefined : endpoint);
        return;
      }
