    ...
    <div className="min-h-screen bg-[#f4f0e8]" data-evidence-path={path.id} data-evidence-mode={path.mode}>
      ...
      <header className="border-b border-foreground/15">
        <div className="article-shell py-12 sm:py-20">
          ...
          <h1 className="mt-6 font-serif text-[clamp(3.2rem,6.8vw,6.5rem)] font-semibold leading-[0.94] tracking-[-0.06em]">
            {path.title}
          </h1>
        ...
        <section id="step-1" className="scroll-mt-28 border-x border-b border-foreground/15 bg-background p-6 sm:p-9">
          <div className="grid gap-8 lg:grid-cols-[12rem_minmax(0,1fr)]">
            ...
            <div>
              ...
              <p className="mt-5 text-base leading-8 text-muted-foreground">{path.storyQuestion}</p>
            ...
            <div>
              ...
              {claims.length > 0 ? (
                  ...
                  {claims.map((claim) => {
                    ...
                    return (
                      <article key={claim.claim_id} id={claim.claim_id} className="border border-foreground/15 bg-background p-5">
                        ...
                        <p className="mt-4 text-sm leading-7">{claim.quote_or_assertion}</p>
                ...
                <div className="mt-5 border border-rose-800/25 bg-rose-50 p-6 text-rose-950">
                  ...
                  <p className="mt-3 text-sm leading-7">这不是“暂时省略脚注”，而是明确禁止把小说场景写回人物史。</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="grid gap-8 lg:grid-cols-[12rem_minmax(0,1fr)]">
            ...
            <div>
              ...
              {sources.length > 0 ? (
                ...
                <p className="mt-5 border border-dashed border-foreground/25 p-6 text-sm leading-7 text-muted-foreground">
            ...
            <div>
              ...
              <div className="mt-7 grid gap-px bg-white/15 sm:grid-cols-2">
                <div className="bg-[#202827] p-5">
                  ...
                  <p className="mt-3 text-sm leading-7 text-[#d7cfc2]">{path.canSay}</p>
                </div>
                <div className="bg-[#202827] p-5">
                  <p className="text-xs font-semibold text-rose-300">不能说</p>
                  <p className="mt-3 text-sm leading-7 text-[#d7cfc2]">{path.cannotSay}</p>
                </div>
              </div>
              <p className="mt-7 border-l-2 border-[#c38a82] pl-4 text-sm leading-7 text-[#bdb9b0]">身份门禁：{path.identityBoundary}</p>
              <p className="mt-5 text-sm leading-7 text-[#bdb9b0]">下一问：{path.nextQuestion}</p>
            </div>
          </div>
        </section>
      </div>
