    ...
    <div className="min-h-screen bg-[#f4f0e8]">
      ...
      <header className="border-b border-foreground/15">
        <div className="personal-shell py-12 sm:py-20">
          ...
          <div className="mt-12 grid gap-10 lg:grid-cols-[0.75fr_1.25fr] lg:items-end lg:gap-20">
            <div>
              <p className="personal-kicker"><span aria-hidden="true" />Reading companion</p>
              <h1 className="mt-7 font-serif text-[clamp(3.7rem,7vw,7rem)] font-semibold leading-[0.9] tracking-[-0.065em]">
                故事从哪里来，
                <br />
                又必须停在哪里。
              </h1>
          ...
          {novelCompanionEntries.map((entry, index) => (
            <article key={entry.id} className="grid gap-7 bg-background p-6 sm:p-8 lg:grid-cols-[12rem_minmax(0,1fr)_18rem] lg:items-start">
              ...
              <div>
                ...
                <p className="mt-5 text-base leading-8 text-muted-foreground">{entry.lead}</p>
                <div className="mt-6 grid gap-4 sm:grid-cols-2">
                  <div className="border-t border-emerald-800/30 pt-4">
                    <p className="text-xs font-semibold text-emerald-900">可以承载</p>
                    <p className="mt-2 text-sm leading-7 text-muted-foreground">{entry.canCarry}</p>
                  </div>
                  <div className="border-t border-rose-800/25 pt-4">
                    <p className="text-xs font-semibold text-rose-900">不能替小说证明</p>
                    <p className="mt-2 text-sm leading-7 text-muted-foreground">{entry.cannotCarry}</p>
                  </div>
                </div>
